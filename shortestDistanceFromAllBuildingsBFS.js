...
function shortestDistanceFromAllBuildings(grid) {
  ...
  let dis = new Array(ROW);
  for(let i = 0; i < ROW; i++) {
    dis[i] = new Array(COL).fill(0);
  }
  ...
  for(let i = 0; i < ROW; i++) {
      for(let j = 0; j < COL; j++) {
        // Chech the space is empty, the min distance from the building is not zero
        // and we can reach of buildings based on neighbors
        if(grid[i][j] === 0 && dis[i][j] !== 0 && num[i][j] === buildings) {
