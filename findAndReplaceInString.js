...
and a target word y.  The rule is that if x starts at position i in the original string S
...
this second operation does nothing because in the original string S[2] = 'c', which
doesn't match x[0] = 'e'.
...
var findReplaceString = function(S, indexes, sources, targets) {
    let hash = {};
    // Check which index we need to replace, the source and target
    for(let i = 0; i < indexes.length; i++) {
        // { 0 : ["a", "eee"], 2: ["cd", "ffff"] }
        let index = indexes[i];
        hash[index] = [sources[i], targets[i]];
    }
    
    let response = '';
    let index = 0;
    while(index < S.length) {
        // Check if we need to replace that index and the string
        // at index start as the source
        if(index in hash && S.startsWith(hash[index][0], index)) {
            // If yes, concatenate the target
            response += hash[index][1];
            // increment the index "source" times, we don't want to lose
            // other characters
            index += hash[index][0].length;
        } else {
            // If not keep the original characters
            response += S[index];
            index++;
        }
    }
