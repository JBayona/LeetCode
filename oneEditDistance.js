...
function onEditDistance(s1, s2) {
  ...
  while (i < len1 && j < len2) {
    if (s1[i] === s2[j]) {
      ...
      if (len1 > len2) {
        // increase variable that controls s1 (delete char from s1)
        i++;
      } else if (len1 < len2) {
        // increase variable that conttrols s2 (delete char from s2)
        j++;
      } else {
        // both has the same length, so we just need to advance (replace)
        i++;
        j++;
      }
    }
