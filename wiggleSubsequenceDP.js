...
var wiggleMaxLength = function (nums) {
  /*
    ...
    obtained so far considering i^{th}i th element as the last
    element of the wiggle subsequence and ending with a rising wiggle.
    
    down[i] refers to the length of the longest wiggle subsequence
    obtained so far considering i^{th}i th element as the last element
    of the wiggle subsequence and ending with a falling wiggle.
    */
  if (nums.length < 2) {
    return nums.length;
  }
  let up = new Array(nums.length).fill(0);
  let down = new Array(nums.length).fill(0);
  for (let i = 0; i < nums.length; i++) {
    for (let j = 0; j < i; j++) {
      // + 1 because we are increasing as we found an alternative number
      if (nums[i] > nums[j]) {
