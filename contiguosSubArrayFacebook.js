...
const countContiguousSubArray = (arr) => {
  ...
  for(let i = 0; i < arr.length; i++) {
    if(!max.length) {
      ...
      while(max.length && arr[max[max.length-1]] < arr[i]) {
        max.pop();
      }
      // + 1 because it´s tracking the previous max
      leftMax[i] = max.length ? max[max.length - 1] + 1 : 0;
      // New max
      max.push(i);
    }
  }
  ...
  for(let i = arr.length; i > 0; i--) {
    if(!max.length) {
      ...
      rightMax[i] = max.length > 0 ? max[max.length - 1] - 1 : max.length-1;
      max.push(i);
    }
  }

  for(let i = 0; i < arr.length; i++) {
    result[i] = (rightMax[i] - leftMax[i]) + 1;
  }
  return result;
}
...
console.log(countContiguousSubArray(arr))
