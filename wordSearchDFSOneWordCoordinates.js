...
const searchWord = (grid, word) => {
  ...
  for(let i = 0; i < grid.length; i++) {
    for(let j = 0; j < grid[i].length; j++) {
      ...
      if(grid[i][j] === word[0]) {
        let tmpArr = [];
        if(dfs(grid, word, i, j, 0, tmpArr, result)) {
          return result;
        }
      }
    }
...
function dfs(grid, word, row, col, count, tmpArr, result) {
  ...
  let found = dfs(grid, word, row + 1, col, count + 1, tmpArr.concat(), result) ||
  dfs(grid, word, row, col + 1, count + 1, tmpArr.concat(), result);
