...
that a point exists at (ai, bi). A point is valid if it shares the same x-coordinate or the same y-coordinate as
your location.

Return the index (0-indexed) of the valid point with the smallest Manhattan distance from your
current location. If there are multiple, return the valid point with the smallest index. If there are no
...
var nearestValidPoint = function(x, y, points) {
    ...
    for (let i = 0; i < points.length; i++) {
        let [xPoint, yPoint] = points[i];
        if (xPoint === x || yPoint === y) {
            let distance = getDistance([x, y], points[i]);
            if (distance < min) {
                min = distance;
                result= i;
            }
...
function getDistance(tupple1, tupple2) {
    let [x1, y1] = tupple1;
    let [x2, y2] = tupple2;
